...
for l = 1:2
    ...
    posTemp = get(hgcf, 'Position');
    if posTemp(2)+posTemp(4) >= screenSize(4)
        posTemp(2) = posTemp(2) - (screenSize(4)-posTemp(4))/2;
    end
    set(hgcf, 'Position', posTemp);
end
...
hsubp = uicontrol(hgcf,'Style', 'pushbutton', 'String', 'add2Arr',...
    'Position', [180 0 60 20], 'Callback', @addHandletoArray);
hsubp = uicontrol(hgcf,'Style', 'pushbutton', 'String', 'Copy2Clipboard',...
    'Position', [240 0 60 20], 'Callback', @copy2clipboard);
%% buttons connections
