function plotFfts2(f, avg, varargin)
...
%   MANDATORY ARUGMENTS:
%   f       :
%   avg     : cell array containing nFrequencySteps-by-nChannels numerical arrays representing ERPs
%
%   OPTIONAL ARGUMENTS:
...
% % % allowedParameterList = {'chanLabels', 'eventLabel', 'title', 'nMaxChanPerAx', 'axisOfEvent', 'EventColors'};
allowedParameterList = {'chanLabels', 'title', 'nMaxChanPerAx', 'axisOfEvent', 'axisTitles', 'legendStr', 'EventColors', 'LW', 'scale'};
chanLabels      = cellstr( int2str( (1:nChan)' ) );
% eventLabel      = [];
mfigTitle       = 'FFT Explorer';
nMaxChanPerAx   = nChan;
axisOfEvent     = [];
axisTitles      = '';
colorOfEvent    = [];
...
%                     eventLabel = parameterValue;
%                 else
%                     error('plotERPs:eventLabel', ...
%                         'Wrong event labels input.');
%                 end
            case 2, %title
...
% % %     eventLabel = cellstr( int2str( (1:nEventType)' ) );
% % % end
% % % 
% % % if nEventType ~= numel(eventLabel)
% % %     error('the amount of event types and labels do not match');
% % % end
...
fftSize = unique( cellfun(@(x) size(x, 1), avg) );
if numel(fftSize) > 1
    error('all ffts should have the same size');
end
if fftSize ~= numel(f)
    error('size of ffts and f do not match');
end
...
% % % legendList = cell(nEventAx, 1);
% % % titleList = cell(nEventAx, 1);
% % % for i = axisList
% % %     eventsOnAx = find(axisOfEvent == i);
% % %     if numel(eventsOnAx) > 1
% % %         legendList{i} = eventLabel(eventsOnAx);
% % %         titleList{i} = '';
% % %     else
% % %         legendList{i} = {};
% % %         titleList{i} = eventLabel{eventsOnAx};
% % %     end
% % % end
...
pbLabelList = {'channels', 'y zoom +', 'y zoom -', 'scale'};
...
plotData;
    ...
    function plotData
        ...
        ySpacingUnit        = max( cellfun(@(x) max(x(:)), avg) );
        ySpacing            = ySpacingFactor*ySpacingUnit;
        ylim                = [0 (nChanPerAx+1/2)*ySpacing];
            ...
            for iApE = 1:nAxToShowPerEvent
                ...
                for iCh = 1:size(yData, 2)
                    ...
                    trace = plot(ax(iAxOfEvent, iApE), ...
                        f, ...
                        yData(:, iCh) + (nChanPerAx-iCh+1/2)*ySpacing, ... - median( yData(:, iCh) ), ... - mean( yData(:, iCh) ), ...
                    ...
                    zeroLine = plot(ax(iAxOfEvent, iApE), ...
                        [f(1) f(end)], ...
                        [(nChanPerAx-iCh+1/2)*ySpacing (nChanPerAx-iCh+1/2)*ySpacing], ...
                        'Color', [0 0 0], ...
                        'linestyle', '-', ...
                        'linewidth', LW/2 ...
                        );
