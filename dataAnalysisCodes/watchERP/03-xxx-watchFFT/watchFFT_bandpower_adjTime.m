...
for iS = 1:nSub,
    ...
    datasetFilename = fullfile(resDir, sprintf('fftDataset_%s_%dfs_adjTime.mat',sub{iS}, targetFS));
        ...
        for iOdd = 1:nOdd,
            ...
            for iFile = 1:size(subset, 1) % multiple file case
                ...
                for iTime = 1:nTimes
                    ...
                    if iTime == 1
                        timePoints = 1 : (timesInSec(1)*targetFS);
                    else
                        timePoints = (timesInSec(iTime-1)*targetFS+1) : (timesInSec(iTime)*targetFS);
                    end
                    for iTrial = 1:nTrials
                        epoch = sig( stimOnsets(iTrial)+timePoints-1, : );
                        
                        fftVals{iData}    = zeros(nChan, 1 );
                       
                        for iCh = 1:nChan,
                            fftVals{iData}(iCh) = bandpower( epoch(:, iCh), targetFS, [freq(iF)-0.5 freq(iF)+0.5] );
                        end
...
for iCh = 1:numel(subsetChannels)
    ...
    filename = sprintf('fftDataset_%s_%dfs_adjTime.csv', subsetChannels{iCh}, targetFS);
