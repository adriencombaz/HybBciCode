...
fm2.lme <- lme(
  psd ~ (stimDuration+I(stimDuration^2))*oddball
  , data = psdDataGp
  , random = list( subject=~stimDuration+I(stimDuration^2), trial=~stimDuration+I(stimDuration^2) )
  , control = list(opt="optim")
  , method = "REML"
)
...
fm2b.lme <- lme(
  psd ~ (stimDuration+I(stimDuration^2))*oddball
  , data = psdDataGp
  , random = list( subject=(~stimDuration+I(stimDuration^2) - 1), trial=~stimDuration+I(stimDuration^2) )
  , control = list(opt="optim")
  , method = "REML"
)
...
fm2.lme <- lme(
  psd ~ (stimDuration+I(stimDuration^2))*oddball
  , data = psdDataGp
  , random =  pdDiag(~(stimDuration+I(stimDuration^2)))
  , control = list(opt="optim")
  , method = "REML"
)
...
psdData$fitted  <- fitted(fm2.lme)
psdData$res     <- residuals(fm2.lme, type="normalized")
pp4c <- ggplot( psdData, aes(stimDuration, psd, colour=oddball) )
pp4c <- pp4c + geom_point() #+ geom_line( aes(stimDuration, sqrtPsd, group=trialInSubAndCond) , linetype=2, size=0.3)
pp4c <- pp4c + geom_line( aes(stimDuration, fitted, group=oddball) )
pp4c <- pp4c + facet_grid( subject~trialInSubAndCond, scale="free_y" )
pp4c <- cleanPlot(pp4c)
pp4c
...
##############################################################################################################################
##############################################################################################################################
fm1C.lme <- update(fm1.lme, cor = corAR1(form=~stimDuration))
fm2C.lme <- update(fm2.lme, cor = corAR1(form=~stimDuration))

plot( fm2C.lme, resid(., type = "normalized") ~ stimDuration, abline = 0 )
plot( fm2C.lme, resid(., type = "normalized") ~ fitted(.), abline = 0 )
plot(ACF(fm2C.lme, maxLag = 15, resType="n"), alpha = 0.01)
