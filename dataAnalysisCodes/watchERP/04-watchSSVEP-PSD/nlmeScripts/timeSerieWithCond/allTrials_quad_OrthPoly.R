...
polymat<-model.matrix(~poly(stimDuration,2), psdData)
colnames(polymat) <- c("int","linT","quadT")
psdDataGp$linT <- polymat[,2]
psdDataGp$quadT <- polymat[,3]

fm1.lme <- lme(
#   psd ~ (stimDuration+I(stimDuration^2))*oddball
  psd ~ (linT+quadT)*oddball
  , data = psdDataGp
#   , random = ~(stimDuration+I(stimDuration^2))|subject/stimDurationFac
  , random = ~(linT+quadT)|subject/trialInSub
...
fm1b.lme <- lme(
  psd ~ (linT+quadT)*oddball
  , data = psdDataGp
  , random = ~quadT|subject/trialInSub
  , control = list(opt="optim")
  , method = "REML"
)
...
# fm1b.lme <- lme(
#   psd ~ poly(stimDuration,2)*oddball
#   , data = psdDataGp
#   , random = ~poly(stimDuration,2)|subject/trialInSub
#   , control = list(opt="optim")
#   , method = "REML"
# )
