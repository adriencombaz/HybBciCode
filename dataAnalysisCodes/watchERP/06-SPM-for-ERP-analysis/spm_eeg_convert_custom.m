function [D targetValue nonTargetValue] = spm_eeg_convert_custom(S, paramfile)
...
stimId = params.realP3StateSeqOnsets;
if iscell(params.realP3StateSeqOnsets)
    nItems      = numel( unique( params.realP3StateSeqOnsets{1} ) );
    nFlashes    = numel( params.realP3StateSeqOnsets{1} );
else
    nItems      = numel( unique( params.realP3StateSeqOnsets ) );
    nFlashes    = numel( params.realP3StateSeqOnsets );
end
flashEvent  = max([D.trials.events.value]);
nFlash      = sum([D.trials.events.value] == flashEvent);
if nFlash ~= nFlashes, error('unexpected number of flash events'); end

NewEventValue   = sum( unique( [D.trials.events.value] ) ) + 1;
targetStateSeq  = params.lookHereStateSeq( params.lookHereStateSeq~=max(params.lookHereStateSeq) );
tempp           = repmat( targetStateSeq, nItems*params.nRepetitions, 1);
targetId        = tempp(:);
if iscell(stimId)
    eventId = double( stimId{1}(:) == targetId(:) ) + NewEventValue;
else
    eventId = double( stimId(:) == targetId(:) ) + NewEventValue;
end

ind = 1;
for iE = 1:numel(D.trials.events)
    if D.trials.events(iE).value == flashEvent
        D.trials.events(iE).value = eventId(ind);
        ind = ind+1;
    end
end
if ind ~= nFlash+1, error('something wrong with the number of flashes'); end
% indEv = find([D.trials.events.value] == flashEvent);
% for iE = 1:numel(indEv)
%     D.trials.events(indEv(iE)).value = eventId(iE);
% end

nonTargetValue = NewEventValue;
targetValue = NewEventValue+1;
