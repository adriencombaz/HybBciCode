...
save('oneValFftDataset.mat', 'oneValFftDataset');


%%

chanList = {'F3', 'Fz', 'F4', 'C3', 'Cz', 'C4', 'P3', 'Pz', 'P4', 'O1', 'Oz', 'O2'};
chanInd  = cell2mat( cellfun( @(x) find(strcmp(oneValFftDataset.chanList{1}, x)), chanList, 'UniformOutput', false ) );

subjects = unique(oneValFftDataset.subject);
nSub = numel(subjects);
conditions = unique(oneValFftDataset.condition);
ncond = numel(conditions);
stimTimes = unique(oneValFftDataset.timeInSec);
...
for iS = 1:nSub
    ...
    subset = oneValFftDataset( ismember(oneValFftDataset.subject, subjects{iS}) & ismember(oneValFftDataset.timeInSec, max(oneValFftDataset.timeInSec)), : );
...
figure
ind = 1;
x = unique(oneValFftDataset.timeInSec);
for iS = 1:nSub
    for iCh = 1:numel(chanList)
        ...
        for iC = 1:ncond
            ...
            for iT = 1:nStimTimes
                ...
                subset = oneValFftDataset( ismember(oneValFftDataset.subject, subjects{iS}) ...
                    & ismember(oneValFftDataset.condition, conditions{iC}) ....
                    & ismember(oneValFftDataset.timeInSec, stimTimes(iT)), : );
...
for iT = 1:nStimTimes
    ...
    for iC = 1:ncond
        ...
        subset = oneValFftDataset( ismember(oneValFftDataset.condition, conditions{iC}) ...
            & ismember(oneValFftDataset.timeInSec, stimTimes(iT)), : );
