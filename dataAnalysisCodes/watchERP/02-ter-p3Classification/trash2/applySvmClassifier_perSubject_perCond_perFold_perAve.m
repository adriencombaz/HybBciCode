function applySvmClassifier_perSubject_perCond_perFold_perAve( iSList, iCTrainList, iCTestList, iFList, iAveList )
...
if isempty(iSList), iSList = 1:nSub; end
if isempty(iCTrainList), iCTrainList = 1:nCond; end
if isempty(iCTestList), iCTestList = 1:nCond; end
if isempty(iAveList), iAveList = 1:nAveMax; end
...
ii = 1;
filename = fullfile( resDir, sprintf('Results-%.2d.txt', ii) );
while exist( filename, 'file') && ii < 20
    ii = ii+1;
    filename = fullfile( resDir, sprintf('Results-%.2d.txt', ii) );
end
if exist( filename, 'file')
    error('that''s a hell of a lot of files that were created already... you might wanna check it out!');
end
...
%================================================================================================================================
for iS = iSList
    for iCTrain = iCTrainList
        for iCTest = iCTestList
            for iAve = iAveList
                ...
                if isempty(iFList), iFList2 = 1:nRuns; else iFList2 = iFList; end
                if nRuns < max(iFList2), error('wrong number of runs'); end
