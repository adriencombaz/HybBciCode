function applyPooledBldaClassifier_perSubject( iS )
...
fprintf(fid, 'subject, conditionTest, nAverages, accuracy, nCorrect, nCued\n');
fprintf('subject, conditionTest, nAverages, accuracy, nCorrect, nCued\n');
...
for iCTest = 1:nCond
    for iAve = 1:nAveMax
        ...
        for iCrossVal = 1:nRuns
            ...
            %==============================================================================
            %==============================================================================
            classifierFilename  = fullfile( resDir, sprintf('pooledBlda-run%d-%.2dAverages.mat', runIdTest(iCrossVal), iAve) );
            ...
            for iRunTest = 1:size( subsetTestIcv, 1 )
                ...
                xTest = apply(classifier.w, xTest);
                xTest = apply(classifier.n, xTest);
                xTest = reshape(xTest, size(xTest, 2)*size(xTest, 1), size(xTest, 3));
                YlatTest = classify(classifier.b,xTest);
        ...
        fprintf( fid, ...
            '%s, %s, %d, %5.2f, %d, %d\n', ...
            sub{iS}, cond{iCTest}, iAve, 100*nCorrect/nCued, nCorrect, nCued );
        fprintf( ...
            '%s, %s, %d, %5.2f, %d, %d\n', ...
            sub{iS}, cond{iCTest}, iAve, 100*nCorrect/nCued, nCorrect, nCued );
