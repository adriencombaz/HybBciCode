function applyPooledSvmClassifier_perSubject_test( iS )
    ...
    for iAve = 1:nAveMax
        for iCTrain = 1:2
            ...
            for iCrossVal = 1:nRuns
                ...
                %==============================================================================
                if iCTrain == 1
                    subsetTrainIcv      = subsetTest( ismember( subsetTest.run, runIdTest(iCrossVal) ), : );
                    ...
                    resDir1             = strrep( resDir, 'LinSvmPooled', 'LinSvm' );
                    classifierFilename  = fullfile( resDir1, filename );
                    classifier          = load(classifierFilename);
                    tag =  cond{iCTest};
                else
                    classifierFilename  = fullfile( resDir, sprintf('pooledSvm-run%d-%.2dAverages.mat', runIdTest(iCrossVal), iAve) );
                    classifier          = load(classifierFilename);
                    tag = 'pooled-classifier';
                end
            ...
            fprintf( fid, ...
                '%s, %s, %s, %d, %5.2f, %d, %d\n', ...
                sub{iS}, tag, cond{iCTest}, iAve, 100*nCorrect/nCued, nCorrect, nCued );
            fprintf( ...
                '%s, %s, %s, %d, %5.2f, %d, %d\n', ...
                sub{iS}, tag, cond{iCTest}, iAve, 100*nCorrect/nCued, nCorrect, nCued );
