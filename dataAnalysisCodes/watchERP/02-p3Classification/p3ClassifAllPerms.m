...
for iS = 1:nSub
    for iC = 1:nCond
        for iAve = 1:nAveMax
            ...
            % read data
            %------------------------------------------------------------------------------
            subsetTrain = subset( ismember( subset.run, 1 ), : );
            ...
            subsetTest = subset( ~ismember( subset.run, 1 ), : );
            
            for iRun = 1:size( subsetTest, 1 )
                
                % read data
                %------------------------------------------------------------------------------
                subsetTesti = subsetTest( iRun, : );
                ...
                nPerms   = nchoosek(nReps, iAve);
                allPerms = combnk(1:nReps, iAve);
                ...
                for iPerm = 1:nPerms
                    ...
                    for iCue = 1:nCues
                        ...
                        for iIcon = 1:nIcons
                            ...
                            iIconFlashes = iStart + find( flashSequence(indEvents) == iIcon );
                            meanCuts( :, :, (iCue-1)*nIcons + iIcon ) = mean( cuts( :, :, iIconFlashes( allPerms(iPerm,:) ) ), 3 );
