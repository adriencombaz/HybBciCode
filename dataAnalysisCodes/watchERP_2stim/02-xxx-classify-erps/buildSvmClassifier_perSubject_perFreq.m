function buildSvmClassifier_perSubject_perFreq( iS, aveList, targetFS, nFoldsSvm  )
...
resDir = fullfile( resDir, sprintf('LinSvm_%dHz_%.2dcvSvm_perFreq', targetFS, nFoldsSvm), sprintf('subject_%s', sub{iS}) );
...
for iCl = 1:nClassifiers
    ...
    for iAve = aveList
        ...
        %==============================================================================
        %==============================================================================
        fprintf('Subject %s, fold %d out of %d, %d averages, %gHz (%d, out of %d)\n', sub{iS}, iCl, nClassifiers, iAve, pars.ssvepFreq(iF), iF, numel(pars.ssvepFreq));
        classifierFilename = 'svm-train';
        for iR = 1:nRunsForTraining
            classifierFilename = sprintf('%s%d', classifierFilename, listTrainRuns{iCl}(iR));
        end
        classifierFilename  = fullfile( resDir, sprintf('%s-%.2dAverages-%sHz.mat',classifierFilename, iAve, strrep(num2str(pars.ssvepFreq(iF)), '.', '-')) );
        ...
        nT_train        = nT_train_iR*nRunsForTraining;
        nNT_train       = nNT_train_iR*nRunsForTraining;
