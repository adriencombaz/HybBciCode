...
function showPlot(sessionDir, bdfFileName, paramFileName, titleStr)
...
%% cut and average target and non-target responses for each ssvep square

for iSq = 1:numel(expParams.ssvepFreq)
    
    % identify all stimuli onset
    ...
    nItems          = numel( unique( expParams.realP3StateSeqOnsets{iSq} ) );
    stimId          = (iSq-1)*nItems + expParams.realP3StateSeqOnsets{iSq};
    ...
    plotERPsFromCutData2( ...
        ...
        'scale', 8, ...
        'title', sprintf('%s-%.2gHz', titleStr, expParams.ssvepFreq(iSq)) ...
        );
