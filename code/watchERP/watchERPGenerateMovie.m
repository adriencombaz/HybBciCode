function watchERPGenerateMovie
...
p300stimulusStatePerFrame   = zeros(1, roundDurationInFrames);
ssvepStimulusStatePerFrame  = zeros(1, roundDurationInFrames);
cueStimulusStatePerFrame    = zeros(1, roundDurationInFrames);

durationSeqInFrames = round( st.sc.stimuli(iP300Stimuli).durationSequenceInSec * FPS );
...
stateSequence       = st.sc.stimuli(iP300Stimuli).stateSequence;
ind = 0;
for iS = 1:numel(stateSequence)
    p300stimulusStatePerFrame( ind+1:ind+durationSeqInFrames(iS) ) = stateSequence(iS);
    ind = ind+durationSeqInFrames(iS);
end

durationSeqInFrames = round( st.sc.stimuli(iSSVEPStimuli).durationSequenceInSec * FPS );
if sum( durationSeqInFrames ) < roundDurationInFrames
    durationSeqInFrames(end) = durationSeqInFrames(end) + roundDurationInFrames -  sum( durationSeqInFrames );
end
stateSequence       = st.sc.stimuli(iSSVEPStimuli).stateSequence;
ind = 0;
for iS = 1:numel(stateSequence)
    ssvepStimulusStatePerFrame( ind+1:ind+durationSeqInFrames(iS) ) = stateSequence(iS);
    ind = ind+durationSeqInFrames(iS);
end
...
for iFr = 1:roundDurationInFrames
    ...
    % SSVEP Square
    iState  = ssvepStimulusStatePerFrame(iFr);
    iTex    = st.sc.stimuli(iSSVEPStimuli).states(iState).views.iTexture;
    if iTex
        figName = fullfile(pixPath, st.sc.textures(iTex).filename);
        [img , ~, ~] = imread( figName );
        img = double(img)/255;
        pos = st.sc.stimuli(iSSVEPStimuli).states(iState).position;
        phase = 2 * pi * st.sc.stimuli(iSSVEPStimuli).states(iState).frequency * (iFr - frameStart - 1)/FPS;
        alpha = ( 1 + cos( phase ) ) / 2;
    ...
    % P3 stim
    iState  = p300stimulusStatePerFrame(iFr);
    iTex    = st.sc.stimuli(iP300Stimuli).states(iState).views.iTexture;
    figName = fullfile(pixPath, st.sc.textures(iTex).filename);
    [img , ~, alpha] = imread( figName );
    img = double(img);
    alpha = double(alpha);
    pos = st.sc.stimuli(iP300Stimuli).states(iState).position;
